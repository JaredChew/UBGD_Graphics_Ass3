...
Wave::Wave(const float& x, const float& y, const float& z, const int& waveWidth, const int& waveHeight, const float& sizeWidth, const float& sizeHeight, const float& heightFrequency, const float& speed) {
...
void Wave::init() {
	...
	for (int i = 0; i < waveHeight; i++) {
		...
		for (int j = 0; j < waveWidth; j++) {
			...
			waveVertices.push_back(startingPosition.x + (j * sizeWidth)); //width - x
			waveVertices.push_back(startingPosition.y); //height - y
			waveVertices.push_back(startingPosition.z + (i * sizeHeight)); //depth - z

			waveTexture.push_back(texCoord0);
			waveTexture.push_back(texCoord1);

			waveVertices.push_back(startingPosition.x + ((j + 1) * sizeWidth)); //width - x
			waveVertices.push_back(startingPosition.y); //height - y
			waveVertices.push_back(startingPosition.z + (i * sizeHeight)); //depth - z
			...
			waveVertices.push_back(startingPosition.x + ((j + 1) * sizeWidth)); //width - x
			waveVertices.push_back(startingPosition.y); //height - y
			waveVertices.push_back(startingPosition.z + ((i + 1) * sizeHeight)); //depth - z
...
void Wave::initTexture(const char* directiory, const int& textureCoverage) {
	...
	Utilities::ResourcesLoader::loadTexture(directiory, textureID);
...
void Wave::draw(const float& affectHeightFrequency, const float& affectSpeed) { //const float& x, const float& y, const float& z, const int& waveWidth, const int& waveHeight, const float& sizeWidth, const float& sizeHeight, const float& heightFrequency, const float& speed, const int& textureCoverage
	...
	for (int i = 1, j = 0, widthCounter = 0; i < waveVertices.size(); i += 3, j++) {
		...
		case 5: //ST - top left
			waveVertices[i] = startingPosition.y + sin(radian + first);
			break;

		case 1: //FT - top right
		case 2: //ST - bottom right
		case 3: //ST - bottom right
			waveVertices[i] = startingPosition.y + sin(radian + second);
			break;
