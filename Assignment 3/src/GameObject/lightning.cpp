...
Lightning::Lightning(const float& startX, const float& startY, const float& startZ, const float& length, const float& height, const float& width, const int& layerSplit) { //const float& height, const float& length, const float& width, const int& layerSplit
...
void Lightning::init() {
	...
	for (int i = 0; i < xPosition.size() - 1; i++) {
		...
		lightningVertices.push_back(startingPosition.x + xPosition[i]);
		lightningVertices.push_back((startingPosition.y + previousPosY) + width); //old lightning split
		//lightningVertices.push_back(yPosition[i] + width); //new lightning Split test
		lightningVertices.push_back(startingPosition.z);

		lightningTexture.push_back(0);
		lightningTexture.push_back(0);

		lightningVertices.push_back(startingPosition.x + xPosition[i + 1]);
		lightningVertices.push_back((startingPosition.y + nextPosY) + width); //old lightning split
...
void Lightning::splitLightning(std::vector<float>& xPosition, std::vector<float>& yPosition, const float& startX, const float& endX, const float& startY, int& currentLayer) {
	...
	const float gradiant = Utilities::StraightLineEquation::gradiantFromTwoPoint(startX, startY, endX, height);

	const float perpendicularGradiant = Utilities::StraightLineEquation::perpendicularGradiant(gradiant);

	const float intersection = Utilities::StraightLineEquation::intersectingPoint(startY, endX, perpendicularGradiant);

	const float randomPosY = Utilities::Random::randBetweenFloat(-height, height);

	const float midpoint = Utilities::StraightLineEquation::xPosAtYPos(randomPosY, perpendicularGradiant, intersection);
